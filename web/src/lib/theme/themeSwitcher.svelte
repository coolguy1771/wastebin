    ...
    for (let i = 0; i < dataThemeButtons.length; i++) {
      dataThemeButtons[i].addEventListener('click', () => {
        ...
        setTheme({
          ...
          'color-text-pri': themeData?.primaryColor,
          'color-text-acc': themeData?.accentColor,
        })
...
{#if showHelp}
  <span class="help"
    >Setting theme for <b>{darkMode ? 'dark' : 'light'}</b> mode. Active mode will be chosen based on
    the browser's preference</span
  >
{/if}
<button class="theme-mode" class:dark={darkMode} on:click={() => (darkMode = !darkMode)}
